...
async function generateAIChallenge(): Promise<{ title: string; description: string; tasks: any[] }> {
  const challengeTypes = [
    'quiz', 'feature_promo', 'invite_friends', 'chart_analysis', 'market_knowledge',
    'vip_feature', 'trading_concept', 'platform_education', 'trading_psychology', 'risk_management',
  ];
  const chosenType = challengeTypes[Math.floor(Math.random() * challengeTypes.length)];

  const prompts: Record<string, string> = {
    quiz: 'Create a forex education quiz challenge. Include 3-4 questions about forex concepts, candlestick patterns, or technical analysis terms. Make it educational and fun.',
    feature_promo: 'Create a challenge to promote the app. Ask users to screenshot the VIP room, share the app link, or show they used a specific feature.',
    invite_friends: 'Create a friend invitation challenge. Ask users to invite friends, share referral links, or post about the platform.',
    chart_analysis: 'Create a trading chart analysis challenge. Ask users to upload and analyze a forex chart, identify patterns, or predict direction.',
    market_knowledge: 'Create a forex market knowledge challenge. Include questions about currency pairs, trading sessions, or economic indicators.',
    vip_feature: 'Create a challenge showcasing VIP features. Ask users to find specific features in the VIP room, describe signals, or use the AI tools.',
    trading_concept: 'Create a trading concepts challenge. Include questions about support/resistance, trend lines, moving averages, or risk management.',
    platform_education: 'Create a challenge that teaches users about the platform features like AVAX AI, Smart Market, or Trading Journal.',
    trading_psychology: 'Create a trading psychology challenge. Ask about emotions in trading, discipline, or common trading mistakes.',
    risk_management: 'Create a risk management challenge. Ask about lot sizes, stop losses, risk/reward ratios, or position sizing.',
  };
  ...
  const { data, error } = await supabase.functions.invoke('avax-ai', {
    body: {
      ...
      messages: [{
        role: 'user',
        content: `${prompts[chosenType]}\n\nGenerate a weekly challenge for VISION AVAX FOREX trading platform.\n\nRespond with ONLY this JSON (no markdown, no code blocks, no extra text):\n{\n  "title": "Short engaging title",\n  "description": "2-3 sentence description",\n  "tasks": [\n    {\n      "task_type": "quiz",\n      "question": "Question text",\n      "description": "Instructions",\n      "example_answer": "Example",\n      "max_score": 25\n    }\n  ]\n}\n\nRequirements: exactly 4 tasks, mix task_type (quiz/image_upload/screenshot/analysis/invite), educational forex content.`,
      }],
      customSystemPrompt: 'You are a forex challenge designer. Output ONLY raw JSON — no markdown, no code fences, no explanation. Just the JSON object.',
      topicName: 'Challenge Designer',
    },
  });
  ...
  try {
    ...
    console.error('AI parse error:', parseErr, 'Raw:', data.text?.slice(0, 300));
...
export default function AdminTradingChallenge() {
  ...
  async function generateAIChallengeManual() {
    ...
    try {
      ...
      toast.success(`✅ AI Challenge generated: "${generated.title}" — Review & publish when ready`);
      fetchAll();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-500/10 to-primary/10 border border-blue-500/25 rounded-2xl p-4">
        ...
        <button onClick={generateAIChallengeManual} disabled={generatingAI}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <div className="space-y-2">
              ...
              {tasks.length === 0 && <p className="text-center text-muted-foreground text-sm py-4">No tasks yet — add manually or use AI generate</p>}
            </div>
          </div>
