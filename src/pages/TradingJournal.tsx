...
async function analyzeImage(imageUrl: string, mode: 'setup' | 'result' = 'setup') {
  const { data: resp } = await supabase.functions.invoke('analyze-signal', {
    body: { imageUrl, mode: mode === 'result' ? 'result' : undefined }
  });
...
function UploadResultModal({ entry, userId, onSave, onClose }: {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (parsed?.result) {
      ...
      setResult(r === 'win' ? 'WIN' : r === 'loss' ? 'LOSS' : 'BREAKEVEN');
    }
    if (parsed?.pips) setPips(Math.abs(parseFloat(parsed.pips)).toString() || '');
    setAnalyzing(false);
    toast.success('AI analyzed result!');
    e.target.value = '';
  }
